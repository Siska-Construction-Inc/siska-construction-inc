...
import { getLatestProject } from "@/data/projects";
...
export function Hero() {
  ...
  const translation = useMemo(() => {
    return latestProject?.translations[locale] ?? latestProject?.translations[defaultLocale];
  }, [locale, latestProject]);

  const primaryHref = latestProject ? `/projects/${latestProject.id}` : "/projects";
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] md:items-center">
        <div className="space-y-8">
          <div className="inline-flex items-center gap-3 rounded-full border border-neutral-200 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-neutral-500 shadow-sm">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link href={primaryHref} className={primaryButtonClasses}>
              {t("hero.ctaPrimary")}
            </Link>
